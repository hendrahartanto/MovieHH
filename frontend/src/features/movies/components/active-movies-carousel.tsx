...
export const ActiveMoviesCarousel = () => {
  ...
  const movies = originalMovies.length > 3
  ...
  useEffect(() => {
    ...
    const handleScrollEnd = () => {
      ...
      const singleSetWidth = originalMovies.length * (firstItem.offsetWidth + 24);
  ...
  const scroll = (direction: "left" | "right") => {
    ...
    const visibleItems = Math.max(1, Math.floor(container.clientWidth / scrollStep));
    const scrollAmount = scrollStep * visibleItems;

    if (direction === "right" && container.scrollLeft + scrollAmount >= singleSetWidth * 2) {
      container.style.scrollBehavior = "auto";
      container.scrollLeft -= singleSetWidth;
  ...
  if (isError || movies.length === 0) {
    return (
      <div className="layout-middle py-12 text-center text-muted-foreground">
